...
// DATA Access 모델 (Dao) : 쿼리하나가 일어날때마다 발생하는것을 처리 (보통CRUD를 정의)
// 서비스 모델 (Service) : 유저가 1번의 서비스를 받을 때 일어나는 복수의 쿼리 처리
// 예) : 웹앱에서 유저가 접속을 할때 마다 마지막 접속일을 기록 -> 로그인 서비스
// (select로 id와 pw가 같은 경우 유저를 반환 + 유저의 마지막 접속일을 수정)
// 1개의 쿼리만 실행하는 서비스도 습관적으로 서비스를 정의하고 구현한다.
// 트랜잭션 관리가 가능하다. (여러 쿼리를 실행하는 하나의 서비스를 커밋과 롤백으로 관리하는 것)
// 서비스는 데이터베이스 접속을 원자화한 dao를 재사용하기 위해 존재하며 서비스 자체가 재사용되기 힘들다.
// service 1개가 1개의 서블릿과 맵핑된다.
public interface UserService {
    // 유저에게 제공하는 서비스 (캡슐화)
    // 유저 : 회원가입, 로그인, 회원정보수정, 비밀번호 변경 ???
    // 관리자가 사용하는 서비스 (오픈)
    // 관리자 : 회원정보수정, 유저리스트, 유저상세, 유저전체수정, 회원등록
    // 공통재공
    // 로그인, 아이디체크,
    int signup(UserDto user) throws Exception;
    int register(UserDto user) throws Exception;    // 회원등록
    ...
    // 열거형이 낫지않아요? 열거형이 좋은데 json으로 변환하는게 귀찮다.
