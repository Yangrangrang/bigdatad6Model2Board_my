    ...
    <main class="container">
        ...
        <h3>깃 시작하기</h3>
            <ol>
                ...
                <li>(리모트 저장소를 깃허브로 사용할거면 echo "# 프로젝트 리드미">> README.md, git branch -m mater main을 헐것을 권장)</li>
                <li>프로젝트에 사용할 파일 생성</li>
        ...
        <ul>
            ...
            <li>
                ...
                <ul>
                    ...
                    <li>Changes to be committed : 인덱스에서 추적하는 파일 (작업이력을 add 한 상태 or 인덱스가 작업이력을 추적)</li>
                    <li>noting to commit, working tree clean : 작업이력과 저장소 이력이 동일 (최신, 작업을 하지 않았다.)</li>
            ...
            <li>작업이력을 수정 (new, modify, delete)을 하면 저장소에 등록하기 전 인덱스에서 처리하는데 작업이력을 인덱스에 올리는(추적하는) 명령어가 git add 파일 </li>
        ...
        <h3>새로운 작업을 할수 있는 브랜치</h3>
        <ul>
            <li>git branch "이름" : 기존의 브랜치를 복사한(깃을 복사와 비슷한 링크를 만든다) 새로운 브랜치가 생성된다.(작업하는 브랜치가 바뀌지 않는다.)</li>
            <li>HEAD : 작업하는 위치 (브랜치 or 커밋)</li>
            <li>git checkout (브랜치 이름, 커밋) : 작업하는 이력을 해당 브랜치나 커밋으로 이동</li>
            <li>새로만든 브랜치로 작업하려면 꼭 checkout으로 브랜치를 선택해야한다.</li>

            <li>git checkout -- (.,*,파일명) : 작업이력을 마지막 저장소 이력과 동일하게 수정 (작업한 내역을 취소)</li>
        </ul>
        <h3>브랜치 통합 머지</h3>
        <ul>
            <li>git merge "브랜치명" : 통합의 기준이 되는 브랜치를 선택 후 (checkout) 해당 명령어를 하면 두 브랜치를 병합한 결과 (commit)가 반환되고 기준이 되는 브랜치가 병합한 결과로 </li>
            <li>merge 통합 : 두개의 다른 이력 (브랜치)을 병합하면 무조건 새로운 이력이 나온다.</li>
            <li>fast-forward merge : 이력이 자손관계면 기준이 되는 조상의 이력을 자손 이력으로 변경하기만 하는 상태</li>
            <li>git pull "저장소명" "브랜치명" : git fetch "저장소명" "브랜치명" 과 git merge "저장소명" / "브랜치명"이 모두 실행되는 명령어 (저장소와 브랜치를 다운받고 병합)</li>
            <li>병합 중 같은 파일에 다른 이력이 있는 경우 병합을 멈추고 충돌을 안내 Unmerged paths(CONFLICT)</li>
        ...
        <ul>
            ...
            <li>github : 깃의 리모트 저장소이며 프로젝트 공유 사이트 이기도 하고 코드를 자동완성하는 인공지능을 제공</li>
            <li>git (add,remove,modify) remote "저장소명(origin)" "url" :깃 저장소에 리모트 저장소 등록 </li>
            <li>git push "저장소명" "브랜치명" : 로컬저장소의 브랜치 이력으로 리모트에 올리는 것</li>
            <li>git fetch "저장소명" "브랜치명" : 리모트의 브랜치를 로컬저장소에 다운 받는 것</li>
            <li>git pull "저장소명" "브랜치명" : 리모트의 브랜치를 로컬저장소에 다운 받고 현재 브랜치와 통합하는 것</li>

            <li>리모트저장소/ 브랜치이름 : 리모트 저장소에서 가져온 브랜치 (git fetch "저장소이름" "브랜치이름")</li>
            <li>git checkout (--track) "저장소이름"/ "브랜치이름" : 리모트 저장소에서 가져온 브랜치를 선택</li>
            <li>git checkout "저장소의 브랜치이름" : 리모트 저장소에서 가져온 브랜치를 새로운 브랜치로 만들어서 선택</li>
        </ul>
        <h3>rebase 통합</h3>
        ...
        <ol>
            <li>리모트 저장소에서 다운 받기 전 해당 폴더 내부에 동일한 이름의 폴더가 없어야 한다.</li>
            ...
            <li>intelliJ : 모듈 > + > 모듈가져오기 > 프로젝트 선택 > 외부모듈에서 maven 선택</li>
