...
<body>
  ...
  <main class="container">
    ...
    <h2>1:N 조인관계에서 N(reply,board_img,board_prefer)을 불러오는 방법</h2>
    <ol>
      <li>
        조인 sql을 작성해서 boardDao 에 서비스 모델을 추가(board_img 권장)
        <ul>
          <li>조인되는 테이블의 서비스가 종속적이어야 한다.</li>
          <li>서비스가 간단해야한다.</li>
        </ul>
      </li>
      <li>서브쿼리로 집계함수를 작성하고 BoardDto에 필드를 추가 (board_prefer 권장)</li>
      <li>
        별도의 dao를 작성하고 Service에서 호출 (Reply 권장)
        <ul>
          <li>복잡한 서비스를 구현하고 재사용 가능(paging,self join,ajax crud)</li>
          <li>fetch lazy를 지원하는 라이브러리(lib) 사용가능</li>
        </ul>
      </li>
    </ol>
    ...
    <div class="d-flex">
      <%if(board.getBoardImgList()!=null){
        for(BoardImgDto boardImg: board.getBoardImgList()){ %>
          <img style="max-width: 200px" class="img-thumbnail"
